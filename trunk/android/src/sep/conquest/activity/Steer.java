...
public class Steer extends Activity implements Observer {
  ...
  // Gets direction commands from control view
  private Button btnUp;
  private Button btnDown;
  private Button btnLeft;
  private Button btnRight;

  // Makes sensors and their values available
  ...
  private mAccSensorEventListener accListener;
  ...
  /**
   * Initializes sensor control by getting the required reference on 
   * the SensorManager and the acceleration sensor.
   * 
  ...
  private void initializeSensorControl() {
    ...
    // Check, if device has at least one acc sensor. If not, sensor control is
    // not possible.
    if (sensList.size() > 0) {
      accSensor = sensList.get(0);
      accListener = new mAccSensorEventListener();
    } else {
  ...
  /**
   * Initialize control elements of the activity and set EventListener.
   */
  private void initializeControlElements() {
    ...
    chkActivate.setOnCheckedChangeListener(new mOnCheckedChangeListener());
  ...
  private final class mAccSensorEventListener implements SensorEventListener {
  ...
  private final class mOnCheckedChangeListener implements
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
      ...
      // If control is activated and on-screen joystick control is selected
