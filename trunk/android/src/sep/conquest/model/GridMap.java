...
public class GridMap {
  ...
  /**
   * Saves all instances of the class MapNode, which haven't been explored yet,
   * but which has got at least one visited neighbour of the type GraphNode
   */
  ...
  /**
   * Saves the smallest and biggest x- and y-coordinates of the MapNodes in the
   * class GridMap.
   */
  private int[] mapBorders = new int[4];

  /**
   * This constant represents the smallest x-coordinate of all MapNodes saved in
   * the attribute mapBorder on position 0.
   */
  private final int MINXVALUEONMAP = 0;

  /**
   * This constant represents the smallest y-coordinate of all MapNodes saved in
   * the attribute mapBorder on position 1.
   */
  private final int MINYVALUEONMAP = 1;

  /**
   * This constant represents the biggest x-coordinate of all MapNodes saved in
   * the attribute mapBorder on position 2.
   */
  private final int MAXXVALUEONMAP = 2;

  /**
   * This constant represents the biggest y-coordinate of all MapNodes saved in
   * the attribute mapBorder on position 3.
   */
  ...
  public String toString() {
    ...
    for (GraphNode node : it) {
      strb.append("(" + node.getXValue() + "," + node.getYValue() + ","
          + node.getNodeType() + ")");
    }
   ...
   * 
   * @param nodeList
   *          The list with all Nodes in the Graph
  ...
  public GridMap makeMapWithList(LinkedList<MapNode> nodeList) {
    ...
    while (it.hasNext()) {
      ...
      newMap.addNode(newNode.getXValue(), newNode.getYValue(), newNode
          .getNodeType());
    }
    return newMap;
  }
   ...
   *         coordinates. Position 0: MinXValue Position 1: MinYValue Position
   *         2: MaxXValue Position 3: MaxYValue
   */
  ...
  /**
   * This method is called by the public method addNode and updates the smallest
   * or biggest coordinates if necessary.
   * 
   * @param xCoordinate
   *          The x-coordinate of the new inserted MapNode
   * @param yCoordinate
   *          The y-coordinate of the new inserted MapNode
   */
   ...
   * node with the coordinates x and y. If a neighbournode does not exist, the
   ...
   * 
   * @param direction
   *          The orientation of the neighbour
   * @param newNode
   *          The node with the possible neighbours
   */
  private void searchAndsetNeighbour(int direction, GraphNode newNode) {
    ...
    case 0:
      actNeighbour = this.mapTree.get(Utility.makeKey(newNode.getXValue() - 1,
          newNode.getYValue()));
  ...
  // public static final Parcelable.Creator<GridMap> CREATOR = new
  // Parcelable.Creator<GridMap>(){

  /*
   * public GridMap createFromParcel(Parcel source) { GridMap newMap = new
   * GridMap(); LinkedList<MapNode> newList = new LinkedList<MapNode>();
   * source.readList(newList, null); for(MapNode noode : newList){
   * newMap.addNode(noode.getXValue(), noode.getYValue(), noode.getNodeType());
   * } return newMap; }
   * 
   * public GridMap[] newArray(int size) { // TODO Auto-generated method stub
   * return null; }
   * 
   * };
   */
