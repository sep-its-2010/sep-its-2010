...
public class HandlerFactory {
  ...
  /**
   * Returns a chain of Handler objects that handle different kinds of broadcast
   * messages.
   * 
  ...
  /**
   * Returns a chain of Handler objects that handle different kinds of bluetooth
   * messages.
   * 
   * Handled types of requests: - PuckOKRequest - PuckRequest
   * 
   * @param puck
   *          The LogicThread of the Puck that should get the chain.
   * @return Reference on first handler of chain.
   ...
   * messages.
   * 
   * Handled types of messages: - StatusUpdateRequest - FailureRequest
   * 
   * @param env
   *          Instance of Environment that should get the chain.
   * @return Reference on first handler of chain.
   */
  public static Handler getEnvironmentBCChain(Environment env) {
    Handler failHndl = new EnvStatusUpdateRequestHandler(null);
    Handler statHndl = new EnvFailureRequestHandler(failHndl);
    return statHndl;
  }

  /**
   * Returns chain of Handler objects that handle different kinds of Puck
   * messages for the simulator.
   * 
   * Handled types of messages: - SimResetHandler - SimLEDHandler -
   * SimStatusHandler - SimSpeedHandler - SimTurnHandler - SimMoveHandler
   * 
   * @param sim
   *          The simulator that received the message.
